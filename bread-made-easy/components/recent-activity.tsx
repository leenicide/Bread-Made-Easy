...
export function RecentActivity({ userId, limit = 10 }: RecentActivityProps) {
  ...
  const items: ActivityItem[] = useMemo(() => {
    const purchaseItems: ActivityItem[] = purchases.map((p) => ({
      ...
      subtitle: `${new Intl.NumberFormat(undefined, { style: "currency", currency: "USD" }).format(p.amount)} • ${formatDistanceToNow(new Date(p.created_at), { addSuffix: true })}`,
    ...
    const bidItems: ActivityItem[] = bids.map((b) => ({
      ...
      type: "bid",
      title: `Placed a bid`,
      subtitle: `${new Intl.NumberFormat(undefined, { style: "currency", currency: "USD" }).format(b.amount)} • ${formatDistanceToNow(new Date(b.created_at), { addSuffix: true })}`,
